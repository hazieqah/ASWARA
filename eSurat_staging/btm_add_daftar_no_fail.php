...
<body>
<?php
  $sql_fail                     =       "SELECT * FROM  kod_no_ruj_fail_btm WHERE aktif = 'Y' ORDER BY id_kod DESC";							
...
<?php
    ...
    <tr>
      ...
      <td align="center"><a href="btm_add_daftar_no_fail.php?id=<?=$id_kod?>&action=edit" title="Kemaskini Maklumat"><img src="../../pic/edit.png" width="25" height="25" /></a></td>
      <td align="center"><a href="btm_add_daftar_no_fail.php?id=<?=$id_kod?>&action=delete" onclick="return confirm('Anda Pasti Ingin Memadam Rekod Ini?');"><img src="../../pic/delete.jpg" width="16" height="16" /></a></td>
    </tr>	
<?php	$no++;
    	} 
     }	?>
...
if ($action                     ==        "tambah")
  ...
  {
    $id                         =           $_GET['id'];
    $sql_get                    =           "SELECT * FROM  kod_no_ruj_fail_btm where aktif = 'Y' AND id_kod = '$id'";							
    ...
    <form id="form" name="form" method="post" action=""  onSubmit="return checkdata();">
      <table width="80%"  border="0" align="center" bgcolor="#588b97">                
        <tr bgcolor="#588b97">
            <td colspan="3" style="color:#CCC;padding:3px"><strong>::Kemaskini No. Fail BTM::</strong></td>     
        </tr>
        <tr>
        <td colspan="3">&nbsp;</td> 
        </tr>
  ...
  else if($action               ==      "delete")
  {
    ...
    $sql_delete                 =       "UPDATE kod_no_ruj_fail_btm SET aktif = 'N' WHERE id_kod = '$id'";
    $result                     =       mysql_query ($sql_delete) or die ("Error in sql_delete : $sql_delete ".mysql_error());   

    print '<script type="text/javascript">'; 
    print 'alert("No. Fail Berjaya Dipadam!")'; 
    print '</script>';
    echo '<meta HTTP-EQUIV="REFRESH" content="0; url=btm_add_daftar_no_fail.php?action=">';
    exit();
  }
...
if (isset ($_POST['submit']))
{
 $no_ruj_fail                   =     $_POST['no_ruj_fail'];
  ...
  $sql_insert                   =     "INSERT INTO kod_no_ruj_fail_btm (no_ruj_fail, keterangan, aktif) VALUES ('$no_ruj_fail', '$keterangan', 'Y')";
...
else if (isset ($_POST['update']))
{
  ...
  $sql_update                   =     "UPDATE kod_no_ruj_fail_btm SET no_ruj_fail = '$no_ruj_fail', keterangan='$keterangan', aktif='Y' WHERE id_kod = '$id_kod'";
  ...
  print '</script>';
  echo '<meta HTTP-EQUIV="REFRESH" content="0; url=btm_add_daftar_no_fail.php?action=edit&id='.$id_kod.'">';
  exit();
}
