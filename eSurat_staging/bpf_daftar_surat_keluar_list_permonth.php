<?php
	...
	include ("../../includefail/connection_eBPFphp"); 
...
?>  
	...
	a:link:after,a:visited:after{content:"(" attr(href) ")";font-size:0%}	
      ...
      <table width="90%">
        ...
        <td align="center">
        <b><div style="font-size:16px">Daftar Surat Menyurat Keluar<br /><font color="red"><?=$full_month?> <?=$tahun?></font></div></b>
        </td>
        <td>&nbsp;    
        </td>
        </tr>
...
.style1 {
	bgcolor="#FF0000"
			...
			$query_info_surat = "SELECT *
								FROM maklumat_daftar_surat_keluar WHERE month(tarikh_daftar) = '$bulan' AND year(tarikh_daftar) = '$tahun' AND  aktif  = 'Y' ORDER BY tarikh_daftar DESC LIMIT $start_from, $limit ";	
	...
	if (mysql_num_rows($result_info_surat)>0)
    {
			...
			?>
				<tr onMouseOver="ChangeColor(this, true);" onmouseout="ChangeColor(this, false);" onclick="DoNav('bpf_daftar_surat_keluar_list_permonth.php?bulan=<?=$bulan?>&action=2&id_daftar=<?=$id_daftar?>&tahun=<?=$tahun?>&jenis_surat=<?=$jenis_surat?>');" class="tr">
...
{  
  $pagLink .="<a href='bpf_daftar_surat_keluar_list_permonth.php?page=".$i."&action=".$action."&no=".$noloop."&bulan=".$bulan."&tahun=".$tahun."&mode=view&action=&id_daftar=".$id_daftar."&jenis_surat=".$jenis_surat."'>".$i."</a>&nbsp;";
  $noloop = $noloop+10;
}
echo $pagLink . "";  
?>
...
<?php   	if($action == '2')
			{
				include ("bpf_daftar_surat_keluar_peryear_kemaskini.php");
